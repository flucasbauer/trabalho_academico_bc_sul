---
title: "SBC Região Sul"
author: "Lucas Freire Bauer Santos"
date: "2023-08-16"
...
grafico_linha <- ggplot(SBC_Total, aes(x = Ano)) +
  ...
  labs(x = "Ano", y = "US$ Bilhões", title = "Saldo da Balança Comercial - Região Sul",
...
cores <- c("SBC PR" = "yellow3", "SBC SC" = "violetred3", "SBC RS" = "turquoise3")

grafico_linha <- ggplot(SBC_Total, aes(x = Ano)) +
  geom_line(aes(y = ExpImpPR, color = "SBC PR"), size = 1.2) +
  geom_line(aes(y = ExpImpSC, color = "SBC SC"), size = 1.2) +
  geom_line(aes(y = ExpImpRS, color = "SBC RS"), size = 1.2) +
  geom_hline(yintercept = 0, color = "black", size = 0.1) +
  labs(x = "Ano", y = "Saldo da Balança Comercial", title = "Saldo da Balança Comercial - Região Sul",
       subtitle = "Exportações - Importações por Estado", caption = "Fonte: Sua Fonte de Dados") +
  scale_y_continuous(labels = function(x) paste0("$", format(x/1000, big.mark = "."))) +
  scale_color_manual(values = cores, labels = c("SBC PR", "SBC SC", "SBC RS")) +  # Aplicando as cores e rótulos definidos
  theme_light() +
  theme(legend.position = "top",
        panel.background = element_rect(fill = "white", color = "white"),
        panel.border = element_rect(color = "black", fill = NA, size = 1),
